  ...
  return (
    <div>
      <h2 id="transaction-heading">
        <Translate contentKey="coffeeApp.transaction.home.title">Transactions</Translate>
        <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
          <FontAwesomeIcon icon="plus" />
          &nbsp;
          <Translate contentKey="coffeeApp.transaction.home.createLabel">Create new Transaction</Translate>
        </Link>
      </h2>
        ...
        {transactionList && transactionList.length > 0 ? (
            ...
            <tbody>
              {transactionList.map((transaction, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>
                    <Translate contentKey={`coffeeApp.Status.${transaction.status}`} />
                  </td>
                  <td>{transaction.bankError}</td>
                  <td>{transaction.receiptNumber}</td>
                  <td>{transaction.description}</td>
                  <td>{transaction.stan}</td>
                  <td>{transaction.order ? <Link to={`order/${transaction.order.id}`}>{transaction.order.id}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="coffeeApp.transaction.home.notFound">No Transactions found</Translate>
            </div>
          )
        )}
      </div>
